  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-6 flex flex-col gap-6">
      ...
      <div className="glass-panel p-8 flex flex-col gap-6 bg-slate-900/60 border border-slate-800">
          ...
          <div className="border border-slate-800 rounded-xl overflow-hidden">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-900/90 text-slate-400 text-[10px] uppercase font-semibold border-b border-slate-800">
                <tr>
                  <th className="p-3">Rule ID & Title</th>
                  <th className="p-3">Status</th>
                  <th className="p-3">Measured</th>
                  <th className="p-3">Required</th>
                  <th className="p-3">NBC Source</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/60 text-slate-300">
                {results.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-900/40">
                    ...
                    <td className="p-3 text-cyan-400 text-[11px]">{r.regulation_source}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Disclaimer */}
        <div className="p-4 rounded-xl bg-slate-950/50 border border-slate-800 text-[11px] text-slate-500 leading-relaxed">
          <strong>Statutory Disclaimer:</strong> This automated algorithmic screening report is
          generated by BuildWise AI for preliminary code verification. Final architectural
          compliance and permit approvals must be validated by certified architects and local municipal
          authorities (e.g. Municipal Corporation / Development Authority).
        </div>
      </div>
    </div>
  )
